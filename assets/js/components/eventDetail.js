...
function fillEventDetails(event) {
  ...
  if (detailLocation) detailLocation.textContent = `${event.city}, ${event.country}`
  ...
  if (totalPrice) totalPrice.textContent = helpers.formatPrice(event.price * quantity)
}
...
function updateQuantityDisplay() {
  ...
  if (totalPrice && currentEvent) {
    totalPrice.textContent = helpers.formatPrice(currentEvent.price * quantity)
  }
}
...
function handleReservation() {
  ...
  if (currentEvent) {
    helpers.showToast(`${quantity} ticket(s) réservé(s) pour ${currentEvent.title} !`, 'success')
    closeEventDetail()
  }
}
