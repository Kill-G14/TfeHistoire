...
export const filters = {
  ...
  filterEvents(events) {
    return events.filter(event => {
      ...
      const matchesSearch = !this.state.searchTerm || 
        event.title.toLowerCase().includes(this.state.searchTerm.toLowerCase()) ||
      ...
      // Filtre par pays
      const matchesCountry = !this.state.country || 
        this.state.country === 'all' || 
        event.country === this.state.country

      // Filtre par catégorie
      const matchesCategory = !this.state.category || 
        this.state.category === 'all' || 
        event.category === this.state.category

      // Filtre par prix minimum
      const matchesPriceMin = !this.state.priceMin || 
        event.price >= parseFloat(this.state.priceMin)

      // Filtre par prix maximum
      const matchesPriceMax = !this.state.priceMax || 
        event.price <= parseFloat(this.state.priceMax)
  ...
  filterEventsCustom(events, searchTerm, country, category) {
    return events.filter(event => {
      ...
      const matchesCountry = !country || country === 'all' || event.country === country
  ...
  populateAllFilters(events, config = {}) {
    ...
    if (document.getElementById(finalConfig.categorySelectId)) {
      ...
      this.populateSelect(finalConfig.categorySelectId, categories, finalConfig.categoryText)
    }
  },
  ...
  attachFilterListeners(config = {}, onFilterChange = null) {
    ...
    const categorySelect = document.getElementById(finalConfig.categorySelectId)
    if (categorySelect) {
