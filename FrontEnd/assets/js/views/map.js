...
async function loadEvents() {
  ...
  if (result.success && result.data) {
    ...
    helpers.showToast(result.message || 'Erreur lors du chargement des événements', 'error')
    renderEventsList()
  }
}
...
function createEventPopup(event) {
  ...
  const routeButton = userPosition && event.latitude && event.longitude
...
function getFilteredEvents() {
  const maxDistance = parseInt(document.getElementById('distanceFilter')?.value || 500)
...
async function showRoute(eventId) {
  ...
  try {
    ...
    const response = await fetch('http://localhost/tfeHistoire/BackEnd/Api/routeApi.php', {
      method: 'POST',
      headers: {
    ...
    if (result.success && result.data && result.data.features && result.data.features.length > 0) {
      ...
      helpers.showToast(
        `Itinéraire calculé : ${distance} km, environ ${duration} min`,
        'success'
      )

      highlightEvent(eventId)
    } else {
      helpers.showToast(result.message || 'Impossible de calculer l\'itinéraire', 'error')
    }
  } catch (error) {
  ...
  if (eventsList) {
    eventsList.addEventListener('click', (e) => {
      ...
      if (e.target.closest('.zoom-event')) {
        const eventId = parseInt(e.target.closest('.zoom-event').dataset.eventId)
        zoomToEvent(eventId)
      }

      // Bouton itinéraire
      if (e.target.closest('.show-route')) {
        const eventId = parseInt(e.target.closest('.show-route').dataset.eventId)
        showRoute(eventId)
      }
